...
const ClientOrdersPage = ({ orders }) => {
  ...
  const statusFilters = Array.from(new Set(orders.map(order => {
    const lastStage = order.execution.length > 0 ? order.execution[order.execution.length - 1].stage : null;
  ...
  const columns: TableColumnType<Order>[] = [
    ...
    },
    {
      ...
      onFilter: (value, record) => {
        const lastStage = record.execution.length > 0 ? record.execution[record.execution.length - 1].stage : null;
    ...
    },
    {
      ...
      dataIndex: 'leadTime',
      key: 'leadTime',
      sorter: (a, b) => new Date(a.leadTime).getTime() - new Date(b.leadTime).getTime(),
