...
const OrderDetailPage = ({ order, executionHistory }) => {
  ...
  return (
    <div>
              ...
              <ScrollArea className="h-[400px] w-full rounded-md">
                {executionHistory.map((execution) => (
                  <div key={execution.id}>
                    ...
                    <p className="text-gray-800 text-sm" suppressHydrationWarning>
                      {formatDate(execution.executionDate, "dd.MM.yy HH:mm")} <span className="text-gray-400">{execution.user.name}</span>
                    </p>
            ...
            <CardContent>
              <div className="flex flex-col md:flex-row">
                <div className="w-full md:w-1/2 px-4 mb-4 md:mb-0">
                  {order ? (
                    <>
                      <Label htmlFor="orderCreatedAt">Дата создания заказа: {formatDate(order.createdAt, "dd.MM.yyyy")}</Label>
                      <Separator className="my-2" />
                      <Label htmlFor="orderLeadTime">Предварительная дата выполнения заказа: {formatDate(order.leadTime, "dd.MM.yyyy")}</Label>
                      <Separator className="my-2" />
      ...
      <DialogModal open={openTempate} onOpenChange={setOpenTempate} orderId={order.id} />
      <DialogModalChangeStages open={open} onOpenChange={setOpen} orderId={order.id} />
    </div>
  );
};
