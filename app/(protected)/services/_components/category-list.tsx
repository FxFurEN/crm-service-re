  ...
  return (
    <div className="w-full md:w-48 md:flex-shrink-0 mt-7">
      <h4 className="mb-4 text-md font-medium leading-none">
        Категории
        <Button className="ml-2 rounded-full " variant="ghost" onClick={onCategoryButtonClick}>
          <Plus size={24}/>
        </Button>
      </h4>
          ...
          <div className="p-4">
            {categories.map((category, index) => (
              <div key={index} className="text-sm">
                {category.name}
                <Button className="ml-2 rounded-full" variant="ghost" onClick={() => onEditCategory(category.id)}>
                  <Pencil />
                </Button>
              </div>
            ))}
